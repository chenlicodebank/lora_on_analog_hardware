...
def main():
    ...
    model = convert_to_analog(model, gen_rpu_config(output_noise_level=general_args.output_noise_level, pcm_model=general_args.pcm_model), tile_module_class=TorchInferenceTile)
    ...
    if training_args.do_eval:
        ...
        for drift in drift_values:
            ...
            # Log the mean and standard deviation metrics
            logger.info(f"Mean metrics for drift_analog_weights = {drift}: {mean_metrics}")
            logger.info(f"Standard deviation metrics for drift_analog_weights = {drift}: {std_metrics}")
